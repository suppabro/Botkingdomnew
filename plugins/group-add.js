...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
  ...
  if (response[users] == 408) throw `_අසාර්ථකයි!_\n\nඅංකය මෑතකදී ඩයල් කර ඇත\nඇතුළත් කළ හැක්කේ හරහා පමණි *${usedPrefix}link* grup`
  ...
  for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
    ...
    let teks = `ආරාධනා කරනවා @${jid.split`@`[0]} ආරාධනා භාවිතා කරන්න...`
    m.reply(teks, null, {
      contextInfo: {
